...
const isValidAcademicUKEmail = (email) => /^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.ac\.uk$/.test(email.toLowerCase());
// Register
exports.register = async (req, res) => {
  try {
    const { email, password } = req.body;

    if (!isValidAcademicUKEmail(email)) {
      return res.status(400).json({ message: "Only @xxx.ac.uk email addresses are allowed" });
    }
